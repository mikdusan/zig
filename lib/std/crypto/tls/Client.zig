...
pub const StreamInterface = struct {
    ...
    /// stream layer.
    pub fn readv(this: @This(), iovecs: []std.posix.iovec_t) ReadError!usize {
        _ = .{ this, iovecs };
        @panic("unimplemented");
    }
    ...
    pub fn writev(this: @This(), iovecs: []const std.posix.iovec_const_t) WriteError!usize {
        _ = .{ this, iovecs };
        @panic("unimplemented");
    }
    ...
    pub fn writevAll(this: @This(), iovecs: []std.posix.iovec_const_t) WriteError!usize {
                        ...
                        .finished => {
                            ...
                            const app_cipher = switch (handshake_cipher) {
                                inline else => |*p, tag| c: {
                                    ...
                                    var both_msgs_vec = [_]std.posix.iovec_const_t{.{
                                        .base = &both_msgs,
                                        .len = both_msgs.len,
                                    }};
...
pub fn writeEnd(c: *Client, stream: anytype, bytes: []const u8, end: bool) !usize {
    ...
    var iovecs_buf: [6]std.posix.iovec_const_t = undefined;
...
fn prepareCiphertextRecord(
    c: *Client,
    iovecs: []std.posix.iovec_const_t,
    ciphertext_buf: []u8,
    bytes: []const u8,
...
pub fn readAtLeast(c: *Client, stream: anytype, buffer: []u8, len: usize) !usize {
    var iovecs = [1]std.posix.iovec_t{.{ .base = buffer.ptr, .len = buffer.len }};
...
pub fn readv(c: *Client, stream: anytype, iovecs: []std.posix.iovec_t) !usize {
...
/// The `iovecs` parameter is mutable because this function needs to mutate the fields in
/// order to handle partial reads from the underlying stream layer.
pub fn readvAtLeast(c: *Client, stream: anytype, iovecs: []std.posix.iovec_t, len: usize) !usize {
    if (c.eof()) return 0;
...
pub fn readvAdvanced(c: *Client, stream: anytype, iovecs: []const std.posix.iovec_t) !usize {
    ...
    var ask_iovecs_buf: [2]std.posix.iovec_t = .{
        .{
            .base = first_iov.ptr,
            .len = first_iov.len,
        },
...
const VecPut = struct {
    iovecs: []const std.posix.iovec_t,
    idx: usize = 0,
    off: usize = 0,
    total: usize = 0,
...
fn limitVecs(iovecs: []std.posix.iovec_t, len: usize) []std.posix.iovec_t {
